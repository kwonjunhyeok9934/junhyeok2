...
- **여행 준비 칸**(`trip_plans.prep`)은 1일째 위에 따로 있다. 항공권·숙소는 떠나기 전에 결제하니 날짜가 여행 기간 밖이다 —
  일차로 묶으면 1일째로 끌려 들어가 날짜가 거짓말이 된다. `splitPrep(plans)` 로 먼저 갈라낸 뒤 나머지만 일차로 묶는다.
  준비 줄의 `date` 는 **결제한 날**이고 가계부에도 그 날짜로 들어간다. 시트에서 '여행 준비' 칩을 고르면 날짜 칸(`#plan-when`)이 열린다.
- 상세는 **같은 내용이면 다시 그리지 않는다**(`signature()`), 준비물 체크는 그 줄만 고친다(`paintPacked()`).
  통째로 다시 그리면 체크할 때마다 화면이 눈에 띄게 흔들린다 — 실시간 메아리로도 한 번 더 그려지기 때문에 둘 다 필요하다.
- 상세를 불러올 때 `trip_plans` 는 `*` 가 아니라 **열 이름을 적어서** 읽는다(`id,trip_id,date,place,memo,prep,…`).
  `*` 로 받으면 SQL 을 덜 실행해 열이 빠져 있어도 조용히 넘어가고, 나중에 저장할 때서야 실패한다 —
  이름을 적어 두면 화면을 열자마자 "schema.sql 전체를 한 번 실행해 주세요" 로 알려 준다.
