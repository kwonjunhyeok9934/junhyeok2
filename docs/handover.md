...
- 실시간은 `pantry-changes` 채널로 따로 뒀다 — 31번 SQL 을 아직 안 돌린 상태에서도 나머지가 멀쩡하도록.
  같은 이유로 `loadPantry` 는 실패를 삼키고 카드 자리에 "31번 섹션을 실행해 주세요" 만 띄운다.

## schema.sql 메모
`supabase/schema.sql` 은 **전체를 몇 번 다시 실행해도** 되게 맞춰 뒀다(빈 DB에도, 이미 쓰던 DB에도).
고친 곳이 네 군데 있었다.
1. 25번 `save_meal` 에 `end $$;` 가 빠져 있었다 — 뒤 섹션(27·29번)이 함수 본문으로 빨려 들어가
   **빈 DB에 전체를 붙여도** 거기서부터 깨졌다.
2. 13·21번이 카테고리 kind 제약을 23번보다 좁게 걸어, 식비 카테고리가 있는 표에 다시 실행하면 막혔다.
...
검증은 빈 DB + 이미 쓰던 DB에 연속 실행해서 에러가 없는지와 **행 수가 안 늘어나는지**를 같이 본다.
...
## 남은 아이디어
사 둔 것: 유통기한 알림 · 담은 것 재구매 추천 · 스크린샷 읽기를 비전 API 로(정확도↑, 유료) · 지출 검색 · 스케줄 반복 · 가계부 CSV 내보내기 · 연간 보기 · 홈 히어로 배경 사진 · 최근 기록 3개 · 여행 탭(일차별 일정·사진, 세계 지도, 홈에 다가오는 여행 D-day 카드, 여행 지출을 거래에 직접 묶기, 여행 만들면 상대에게 알림)
