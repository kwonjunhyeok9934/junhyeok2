...
아래 탭바는 네 칸이고 그 안의 화면은 여덟 개다. 주소의 `#ledger` `#meal` `#fixed` `#schedule` `#todo` `#travel` `#trips` 중 예전 것은 그대로라
...
- **일정** 은 2박 3일이면 세 칸이다. 칸마다 *어디를 갔고 얼마를 썼는지* 줄을 계속 더한다.
  금액을 적으면 **가계부에 '여행' 지출로 함께 들어간다** (식비와 같은 방식 — 줄 하나 = 거래 하나,
  줄을 지우면 그 거래도 사라진다). 그래서 여행 지출을 두 번 적을 일이 없다.

**여행 탭을 처음 쓸 때 한 번**: Supabase SQL Editor 에 `supabase/schema.sql` 의 27·29·31번 섹션(또는 전체)을 실행해 표를 만든다.
