...
| `js/trip.js` | 여행 탭 · 내 여행 (여행 목록·상세·준비물) |
...
아래 탭바는 네 칸이고 그 안의 화면은 아홉 개다. 주소의 `#ledger` `#meal` `#fixed` `#pantry` `#schedule` `#todo` `#travel` `#trips` 중 예전 것은 그대로라
...
여행 하나는 **제목 + 기간 + 지역 여러 개**다(제주 여행이면 제주시·서귀포시). 여행을 만들면 그 지역이 지도에 자동으로 칠해진다.
여행 상세에는 D-day, 메모, 준비물 체크리스트, 그리고 **그 기간의 가계부 합계**가 있다 — 여행 지출을 따로 적지 않고 가계부에서 더해 보여 준다.

**여행 탭을 처음 쓸 때 한 번**: Supabase SQL Editor 에 `supabase/schema.sql` 의 27·29번 섹션(또는 전체)을 실행해 표를 만든다.
...
**사둔것 탭을 처음 쓸 때 한 번**: `supabase/schema.sql` 의 31번 섹션(또는 전체)을 실행해 표를 만든다.
